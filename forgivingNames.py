...
and our options are 
    ["someOtherThing","doTheBest","doTheBestThing","do_the_best_thing"]
...
Supports:
   ...
   * textual numbers "do thing one" => "doThing1"
     or "do thing 1" => "doThingOne"
   * always tries to eat the largest possible match "do the best thing",
     given ["doTheBest","doTheBestThing"] => "doTheBestThing"
   * prefers caps that match, but will accept those that don't
   * also supports variable stacks, so for programming uses it is
     easy to manage context
"""
import re
import typing
...
numeridecode_re=re.compile(
    r"""(?P<textual>"""+('|'.join(numbers))+r""")|(?:0*(?P<numerical>[1-9][0-9]*))""", # noqa: ignore=E501
    re.IGNORECASE)
        ...
        def match_util(name:str,tape:str,idx:int=0)->float:
            ...
            while tapeIdx<len(tape) and nameIdx<len(name):
                if tape[tapeIdx]==name[nameIdx]:
                    ...
                    tapeIdx+=1
                    nameIdx+=1
                elif tape[tapeIdx] in self.breakingChars \
                    or name[nameIdx] in self.breakingChars:
                    # breaking characters are different
                    # so don't add to the rank, but skip them and keep going
                    if tape[tapeIdx] in self.breakingChars \
                        and name[nameIdx] in self.breakingChars:
                        # mismatched breaking chars are still breaking chars,
                        # so worth a little something
                        rank+=0.5
                    else:
                        rank-=0.5
...
class ForgivingNames:
    ...
    Supports:
        ...
        * prefers caps that match, but will accept those that don't
        * also supports variable stacks, so for programming uses it is
          easy to manage context scope
    """

    def __init__(self):
        self.stackPenaltyFactor=0.10 # parent in the stack is this much less likely to match # noqa: ignore=E501
        self.contextStack:typing.List[ForgivingNamesContext]=\
            [ForgivingNamesContext()]
    ...
    def push(self,
        ctx:typing.Optional[ForgivingNamesContext]=None
        )->ForgivingNamesContext:
        """
...
def cmdline(args:typing.Iterable[str])->int:
    ...
    if printhelp or not didSomething:
        ...
        print('OPTIONS:')
        print('  -h ................ this help')
        print('  --add=name ........ add a new name to the matchable names')
        print('  --push ............ push a new name context onto the stack')
        print('  --pop ............. pop a name context off the stack')
        print('  --find=name  ...... find a name in the current values')
